...
use x509_cert::{Certificate as X509Certificate, der::Decode};
...
impl CertificateAuthority {
    ...
    pub fn new(common_name: &str, organization: &str, validity_days: u32) -> Result<Self> {
        // Generate key pair
    ...
    pub fn from_pem(cert_pem: &str, key_pem: &str) -> Result<Self> {
        let key_pair =
            KeyPair::from_pem(key_pem).map_err(|e| CryptoError::InvalidPem(e.to_string()))?;
    ...
    pub fn issue_server_certificate(
        ...
        let mut sans: Vec<SanType> = san_dns
            .iter()
            .filter_map(|s| s.clone().try_into().ok().map(SanType::DnsName))
            .collect();
        for ip in san_ips {
    ...
    pub fn issue_client_certificate(
        ...
        let cert = params
            .signed_by(&client_key, &self.ca_cert, &self.key_pair)
            .map_err(|e| CryptoError::CertificateError(e.to_string()))?;
...
impl Certificate {
    ...
    pub fn validate_expiration(&self) -> Result<()> {
        let pem_data = pem::parse(self.cert_pem.as_str()).map_err(|e| {
            CryptoError::InvalidPem(format!("Failed to parse certificate PEM: {}", e))
        })?;

        let cert = X509Certificate::from_der(pem_data.contents()).map_err(|e| {
    ...
    pub fn validate_chain(&self) -> Result<()> {
        ...
        let cert_pem_data = pem::parse(self.cert_pem.as_str()).map_err(|e| {
            CryptoError::InvalidPem(format!("Failed to parse certificate PEM: {}", e))
        })?;

        let cert = X509Certificate::from_der(cert_pem_data.contents()).map_err(|e| {
            CryptoError::CertificateError(format!("Failed to parse certificate: {}", e))
        })?;

        // Parse the CA certificate
        let ca_pem_data = pem::parse(self.ca_pem.as_str()).map_err(|e| {
            CryptoError::InvalidPem(format!("Failed to parse CA certificate PEM: {}", e))
        })?;

        let ca_cert = X509Certificate::from_der(ca_pem_data.contents()).map_err(|e| {
            CryptoError::CertificateError(format!("Failed to parse CA certificate: {}", e))
        })?;
