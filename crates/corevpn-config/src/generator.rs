...
use corevpn_crypto::{Certificate, CertificateAuthority};
...
impl ConfigGenerator {
    ...
    pub fn generate_client_config(
        ...
        // Build client config
        let mut builder =
            ClientConfigBuilder::new(username, &self.server_config.server.public_host)
...
impl GeneratedConfig {
    ...
    pub fn save(&self, dir: &Path) -> Result<std::path::PathBuf> {
        ...
        let canonical_dir = dir.canonicalize().map_err(crate::ConfigError::IoError)?;
        ...
        if let Some(parent) = path.parent() {
            ...
            let canonical_parent = parent.canonicalize().map_err(crate::ConfigError::IoError)?;
...
mod tests {
    ...
    fn test_initialize_pki() {
        ...
        let (_ca, ta_key) = initialize_pki(dir.path(), "vpn.example.com", "Test Org").unwrap();
